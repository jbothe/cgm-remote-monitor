...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  if (glucose_data.length > 0) {
    ...
    $('<td class="tdborder"><center>').appendTo(tr);
    $('A ' + (Math.round(10 * (ss.mean(mgDlBgs) + 46.7) / 28.7) / 10).toFixed(1) + '%<sub>DCCT</sub>&nbsp;|&nbsp;' + Math.round(((ss.mean(mgDlBgs) + 46.7) / 28.7 - 2.15) * 10.929) + '<sub>IFCC</sub>').appendTo(tr);
    if (weighted != ss.mean(mgDlBgs)) {
      $(' B<br />' + (Math.round(10 * (weighted + 46.7) / 28.7) / 10).toFixed(1) + '%<sub>DCCT</sub>&nbsp;|&nbsp;' + Math.round(((weighted + 46.7) / 28.7 - 2.15) * 10.929) + '<sub>IFCC</sub>').appendTo(tr);
    }
    $(' C</center></td>').appendTo(tr);
  } else {
    $('<td class="tdborder">N/A</td>').appendTo(tr);
    $('<td class="tdborder">N/A</td>').appendTo(tr);
    $('<td class="tdborder">N/A</td>').appendTo(tr);
