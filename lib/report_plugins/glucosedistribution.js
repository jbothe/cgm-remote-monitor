...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var wtotal = 0;
  var wdivisor = 0.001;
//  var latest_time = glucose_data[glucose_data[glucose_data.length - 1]].displayTime.getTime();
  for (i = 1; i <= glucose_data.length - 2; i++) {
    let entry = glucose_data[i];
//    let entryTime = entry.displayTime.getTime();
    let nextEntry = glucose_data[i + 1];
//    let timeDelta = nextEntry.displayTime.getTime() - entryTime;
    let timeDelta = nextEntry.displayTime.getTime() - entry.displayTime.getTime();
    if (timeDelta < maxGap) {
      timeTotal += timeDelta;
    }
//    let w = 1 - (latest_time - entryTime) / (1000 * 60 * 60 * 24 * 5);
    let w = 1 - (glucose_data[glucose_data[glucose_data.length - 1]].displayTime.getTime() - entry.displayTime.getTime()) / (1000 * 60 * 60 * 24 * 5);
    if (w > 0) {
...
//    let day =  (latest_time - entryTime) / (1000 * 60 * 60 * 24);
//    if (day < 120) {
//      let w = (1 - day/120);
  ...
  if (glucose_data.length > 0) {
    ...
    $('<td class="tdborder"><center>' + (Math.round(10 * (ss.mean(mgDlBgs) + 46.7) / 28.7) / 10).toFixed(1) + '%<sub>DCCT</sub>&nbsp;|&nbsp;' + Math.round(((ss.mean(mgDlBgs) + 46.7) / 28.7 - 2.15) * 10.929) + '<sub>IFCC</sub><br />' + (Math.round(10 * (weighted + 46.7) / 28.7) / 10).toFixed(1) + '%<sub>DCCT</sub>&nbsp;|&nbsp;' + Math.round(((weighted + 46.7) / 28.7 - 2.15) * 10.929) + '<sub>IFCC</sub></center></td>').appendTo(tr);
  } else {
    $('<td class="tdborder">N/A</td>').appendTo(tr);
    $('<td class="tdborder">N/A</td>').appendTo(tr);
    $('<td class="tdborder">N/A</td>').appendTo(tr);
